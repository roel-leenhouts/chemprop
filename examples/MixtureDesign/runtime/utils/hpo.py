...
def build_gnn_search_space(profile: str = "default") -> dict[str, Any]:
    if profile == "smoke":
        return {
            ...
            "final_lr": choice([1e-4]),
            "warmup_epochs": choice([1]),
        }
    if profile != "default":
    ...
    return {
        ...
        "init_lr": choice([1e-4, 3e-4]),
        "max_lr": loguniform(3e-4, 3e-3),
        "final_lr": choice([5e-5, 1e-4, 3e-4]),
        "warmup_epochs": choice([1, 2, 4]),
    }
