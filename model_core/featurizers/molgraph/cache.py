...
from model_core.featurizers.base import Featurizer, S
from model_core.utils import parallel_execute


class MolGraphCacheFacade(Sequence[MolGraph], Generic[S]):
    """
    A :class:`MolGraphCacheFacade` provided an interface for caching
    :class:`~model_core.data.molgraph.MolGraph`\s.
...
class MolGraphCache(MolGraphCacheFacade):
    ...
    :class:`~model_core.data.molgraph.MolGraph`\s and caches them in memory.
    """
...
class MolGraphCacheOnTheFly(MolGraphCacheFacade):
    ...
    :class:`~model_core.data.molgraph.MolGraph`\s as they are requested.
    """
