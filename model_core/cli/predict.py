...
from model_core import data, featurizers
...
from model_core.models.utils import load_model, load_output_columns
from model_core.nn.message_passing import BondMessagePassing
from model_core.nn.metrics import BoundedMixin
from model_core.nn.predictors import EvidentialFFN, MulticlassClassificationFFN, MveFFN, QuantileFFN
from model_core.uncertainty import (
...
class PredictSubcommand(Subcommand):
    COMMAND = "predict"
    HELP = "Use a pretrained model for prediction."
...
def add_predict_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--preds-path",
        type=Path,
        help="Specify path to which predictions will be saved. If the file extension is .pkl, it will be saved as a pickle file. Otherwise, predictions will be saved as a CSV. If multiple models are used to make predictions, the average predictions will be saved in the file, and another file ending in '_individual' with the same file extension will save the predictions for each individual model, with the column names being the target names appended with the model index (e.g., '_model_<index>').",
    )
    parser.add_argument(
        "--drop-extra-columns",
        action="store_true",
