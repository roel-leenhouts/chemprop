...
from model_core.cli.common import add_common_args, process_common_args, validate_common_args
from model_core.cli.train import (
    TrainSubcommand,
    add_train_args,
    build_datasets,
    build_MAB_model,
...
from model_core.cli.utils.command import Subcommand
from model_core.data import MolAtomBondDataset, build_dataloader
from model_core.nn import AggregationRegistry, MetricRegistry
from model_core.nn.transforms import UnscaleTransform
from model_core.nn.utils import Activation
...
def add_hpopt_args(parser: ArgumentParser) -> ArgumentParser:
    hpopt_args = parser.add_argument_group("Model hyperparameter optimization arguments")
...
def process_hpopt_args(args: Namespace) -> Namespace:
    if args.hpopt_save_dir is None:
        args.hpopt_save_dir = Path(f"model_core_hpopt/{args.data_path.stem}")
...
def tune_model(
    ...
    match args.raytune_search_algorithm:
        ...
        case "hyperopt":
            if NO_HYPEROPT:
                raise ImportError(
                    "HyperOptSearch requires hyperopt to be installed. Use 'pip install -U hyperopt' to install or use 'pip install -e .[hpopt]' in model_core folder if you installed from source to install all hpopt relevant packages."
                )
        ...
        case "optuna":
            if NO_OPTUNA:
                raise ImportError(
                    "OptunaSearch requires optuna to be installed. Use 'pip install -U optuna' to install or use 'pip install -e .[hpopt]' in model_core folder if you installed from source to install all hpopt relevant packages."
                )
...
def main(args: Namespace):
    if NO_RAY:
        raise ImportError(
            "Ray Tune requires ray to be installed. If you installed the package from PyPI, run 'pip install -U ray[tune]' to install ray. If you installed from source, use 'pip install -e .[hpopt]' in the package folder to install all hpopt-relevant packages."
        )
    ...
    if args.tracking_metric != "val_loss":  # i.e. non-default
        if any(
            ...
            raise NotImplementedError(
                "`val_loss` is the only implemented tracking metric for hpopt when training on atom and bond targets. Open an issue on the GitHub repo if you would like us to add support for other tracking metrics in hpopt: https://github.com/model_core/model_core/issues"
            )
