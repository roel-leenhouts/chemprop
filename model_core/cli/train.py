...
from model_core.cli.common import (
    add_common_args,
    find_models,
    process_common_args,
    validate_common_args,
)
from model_core.cli.conf import CHEMPROP_TRAIN_DIR, NOW
from model_core.cli.utils import (
    LookupAction,
    Subcommand,
...
from model_core.cli.utils.args import uppercase
from model_core.conf import LIGHTNING_26_COMPAT_ARGS
from model_core.data import (
    MolAtomBondDataset,
    MoleculeDataset,
    MolGraphDataset,
...
from model_core.data.datasets import _MolGraphDatasetMixin
from model_core.featurizers.atom import AtomFeatureMode
from model_core.models import MPNN, MolAtomBondMPNN, MulticomponentMPNN, save_model
from model_core.nn import AggregationRegistry, LossFunctionRegistry, MetricRegistry, PredictorRegistry
from model_core.nn.ffn import ConstrainerFFN
from model_core.nn.message_passing import (
    AtomMessagePassing,
    BondMessagePassing,
...
from model_core.nn.transforms import GraphTransform, ScaleTransform, UnscaleTransform
from model_core.utils import Factory
...
def add_train_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--save-dir",
        type=Path,
        help="Directory where training outputs will be saved (defaults to ``CURRENT_DIRECTORY/model_training/STEM_OF_INPUT/TIME_STAMP``)",
    )
    parser.add_argument(
        "--remove-checkpoints",
        action="store_true",
    ...
    train_args.add_argument(
        ...
        help="Specify the evaluation metrics. If unspecified, the package will use the following metrics for given dataset types: regression -> ``rmse``, classification -> ``roc``, multiclass -> ``ce`` ('cross entropy'), spectral -> ``sid``. If multiple metrics are provided, the 0-th one will be used for early stopping and checkpointing.",
    )
    train_args.add_argument(
        "--tracking-metric",
        default="val_loss",
