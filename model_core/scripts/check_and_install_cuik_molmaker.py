...
# requests is needed for this install but is not part of a typical model_core install
try:
    import requests
except ImportError:
    print(
...
if not os.path.exists(rdkit_so_file):
    ...
    print(
        "This probably means that RDKit was not installed using conda. Currently, cuik-molmaker is only supported with RDKit installed using conda. Please install RDKit and the project dependencies from conda and try again."
        "For installation instructions, please refer to the package documentation: https://model_core.readthedocs.io/en/latest/installation.html#option-3-installing-from-source-using-environment-yml"
    )
    exit(1)
...
if not os.path.exists(libtorch_so_file):
    ...
    print(
        "This probably means that PyTorch was not installed using conda. Currently, cuik-molmaker is only supported with PyTorch installed using conda. Please install PyTorch and the project dependencies from conda and try again."
        "For installation instructions, please refer to the package documentation: https://model_core.readthedocs.io/en/latest/installation.html#option-3-installing-from-source-using-environment-yml"
    )
    exit(1)
...
print("You can now use cuik-molmaker with this package via --use-cuikmolmaker-featurization.")
